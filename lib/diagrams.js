...
module.exports = {
    ...
    config: {
        previewerWidth: {
            title: 'Diagram Previewer Width',
            description: 'The width of the preview window',
            type: 'integer',
            default: 600,
            minimum: 200
        }
...
function setupPreviewer(editor){
    ...
    if( atom.workspace.isTextEditor( editor ) &&
        ...
        // Poll for changes in the editor container size or the previewer width settings
        // the so we can adjust the previewer and scroll window accordingly
        let lastContainerWidth = null;
        let lastPreviewerWidth = null;
        function checkForResize() {
            ...
            if( editor.element.parentElement !== null &&
                (
                    lastContainerWidth !== editor.element.parentElement.clientWidth ||
                    lastPreviewerWidth !== atom.config.get('diagrams.previewerWidth')
                ) ) {

                previewElement.style.width = `${atom.config.get('diagrams.previewerWidth')}px`;

                const remainingWidth = Math.max(0, editor.element.parentElement.clientWidth - atom.config.get('diagrams.previewerWidth') );
        ...
        const debouncedSequenceUpdate = debounce( 100, () => {
            const Diagram = require('./sequence-diagram.js');
        ...
        const debouncedFlowchartUpdate = debounce( 100, () => {
            const oldRaphel = window.Raphael;
            window.Raphael = require('./raphael.js');
            const flowchart = require('flowchart.js');
            window.Raphael = oldRaphel;
            ...
            try{
                */
                chart = flowchart.parse(editor.getBuffer().getText());
            /*} catch(err){
                const matches = err.message.match(/(.*) on line (\d*):/);
                const lineNumber = parseInt(matches[2])-1;
                const errorDescription = matches[1];
