            ...
            switch( path.extname( editor.getPath() ) ) {
                case ".dot":
                    process.nextTick( () => {
                        ...
                        viz.renderSVGElement( editor.getBuffer().getText() )
                        .then(function(element) {
                            previewers[editor._previewerId]._previewer.element.innerHTML = element.outerHTML;
                        })
                        .catch(err => {
                            const matches = err.stack.match(/Error: ((.*) in line (\d*))/);
                        		const description = matches[1];
                        		const lineNumber = parseInt(matches[3])-1;
                            markError(previewers[editor._previewerId], lineNumber, description);
                        });
                    });
                    break;
                case ".sequence":
